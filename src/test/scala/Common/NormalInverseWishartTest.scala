...
class NormalInverseWishartTest extends AnyFunSuite {
  ...
  test("Update NIW with data") {
    {
      ...
      val n = 100
      val data = MultivariateGaussian(DenseVector(0D, 0D), DenseMatrix.eye[Double](2))(randbasis).sample(n).toList
  ...
  test("Normal Inverse Wishart density") {
    {
      ...
      val mv = MultivariateGaussian(DenseVector(1, 0),
        1.2 * DenseMatrix.eye[Double](2))(randbasis)
